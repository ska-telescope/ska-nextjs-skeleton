...
describe('ContactForm', () => {
  it('renders with all props should return button with appropriate attributes', () => {
    ...
    expect(labels[0].innerHTML).toBe('<span class="label-text">Your name</span>');
    expect(labels[1].innerHTML).toBe('<span class="label-text">Your email</span>');
    expect(labels[2].innerHTML).toBe('<span class="label-text">Your message</span>');
    // Two input box
    const textboxes = document.querySelectorAll('input');
    expect(textboxes?.length).toBe(2);
    expect(textboxes[0].getAttribute('class')).toBe('input input-bordered w-full max-w-xs');
    expect(textboxes[0].getAttribute('type')).toBe('text');
    expect(textboxes[0].getAttribute('name')).toBe('name');
    expect(textboxes[1].getAttribute('class')).toBe('input input-bordered w-full max-w-xs');
    expect(textboxes[1].getAttribute('type')).toBe('text');
    expect(textboxes[1].getAttribute('name')).toBe('email');
    // One TextArea
    const textarea = document.querySelectorAll('textarea');
    expect(textarea?.length).toBe(1);
    expect(textarea[0]?.getAttribute('class')).toBe('textarea textarea-bordered h-24');
